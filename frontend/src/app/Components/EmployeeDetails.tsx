...
import { getEmployeeById, updateEmployee, CreateEmployeeRequest } from "@/lib/api";
...
export default function UpdateForm() {
    ...
    React.useEffect(() => {
        if (!employeeId) {
            setErrors({ general: "No employee ID provided" });
            setLoading(false);
            return;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateEmployee(Number(employeeId), form);
            
            // Refresh the employees table if the function exists
            ...
            console.error(err);
            setErrors({ general: "Failed to update employee. Please check your input and try again." });
        } finally {
    ...
    if (loading) return <div className="py-4 text-center">Loading employee data...</div>;
